...
namespace CapaDatos
{
    public class DUsuario
    {
        ...
        public Respuesta<EUsuario> LoginUsuarioWeb(string Usuario, string Clave, string Token)
        {
            try
            {
                EUsuario obj = null;
                ...
                using (SqlConnection con = ConexionBD.GetInstance().ConexionDB())
                {
                    using (SqlCommand comando = new SqlCommand("usp_LogeoUsuarioZero", con))
                    {
                        comando.CommandType = CommandType.StoredProcedure;
                        comando.CommandTimeout = 30;
                        comando.Parameters.AddWithValue("@Users", Usuario);
            ...
            try
            {
                ...
                using (SqlConnection con = ConexionBD.GetInstance().ConexionDB())
                {
                    using (SqlCommand cmd = new SqlCommand("usp_ConsultaToken", con))
                    {
                        ...
                        SqlParameter outputParam = new SqlParameter("@Token", SqlDbType.NVarChar, 500) // Tamaño fijo en lugar de 'max'
                        {
                            Direction = ParameterDirection.Output
                        };
                        cmd.Parameters.Add(outputParam);
